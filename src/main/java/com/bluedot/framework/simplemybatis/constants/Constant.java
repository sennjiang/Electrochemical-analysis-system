...
/**
 * @version: 1.0
 * @Description: 配置文件和标签的名称常量
 * 存储字符串常量，配置文件使用的时properties文件，配置文件内所有的key在该类中都有一个字符串常量来表示，在读取配置文件信息时也使
 * 用Constant类中的常量作为key来获取。对于一些逻辑代码需要用到的字符串常量也在此处定义 Constant.java:
 * @Author: KangLongPing
 * @Date: 2021/8/20 9:30
 */
