...
public class MyDataSourceImpl implements MyDataSource {
	...
	private MyDataSourceImpl() {
		/**
		 *防止破坏单例
		 * 防止反向通过反向实例化对象而路过getInstance方法
		 * 只能在已通过getInstance方法创建对象后起作用
		 * 如果一开始就使用反射创建对象的话, 由于instance对象并没有被实例化 , 所以能够一起反制创建 对象
		 * 要想使用反射创建必须满足instance对象为空, Configuration类中已经加载了配置文件
		 */
		if (instance != null) {
			// 对象已经实例化, 拒绝通过反射创建
			LOGGER.error("Object has been instanced,reject creating Object by Reflect!");
		}
		init();
	}
	...
	private void autoReduce(Connection conn) {
		synchronized (MONITOR) {
			if (createdCount > minCount && conns.contains(conn)) {
				...
				LOGGER.debug("所归还连接" + conn + "保留在连接池中或已被使用, 当前已创建连接数量: " + createdCount + "当前空闲连接数" + conns.size());
			}
		}
	}
	...
	public Connection getConnection() {
		...
		synchronized (MONITOR) {
			if (conns.size() > 0) {
				LOGGER.debug("获取到连接: " + conns.peek() + "当前已创建的连接数量" + createdCount + "当前空闲连接数" + (conns.size() - 1));
				return conns.poll();
			}
			// 如果没有空连接, 则调用自动增长方法
			if (createdCount < maxCount) {
